...
pub fn get_with_params(parameters: GetConstructorStandingsOpts) -> SelectStatement {
    ...
    if let Some(round) = parameters.round {
        query = query.cond(Expression::eq("race.round", round));
    } else {
        query = query.cond(Expression::eq(
    ...
    if let Some(year) = parameters.year {
        query = query.cond(Expression::eq("race.season.year", year));
    }

    if let Some(constructor_ref) = parameters.constructor_ref {
        query = query.cond(Expression::eq("constructors.ref", constructor_ref));
    }

    if let Some(position) = parameters.position {
        query = query.cond(Expression::eq("position_text", position));
    }
